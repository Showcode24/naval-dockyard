...
import { marineLogisticsData } from "@/data/services/marine-logistics"
    ...
    <>
      ...
      >
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            ...
            >
              Comprehensive maritime logistics solutions supporting vessel operations and supply chain management
            </motion.p>
          </div>
        </div>
      </motion.section>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20" ref={overviewRef}>
            <motion.div initial="hidden" animate={overviewInView ? "visible" : "hidden"} variants={fadeInUp}>
              <h2 className="text-3xl font-bold mb-6">Strategic Maritime Logistics</h2>
              <p className="text-lg text-muted-foreground mb-6">{marineLogisticsData.overview}</p>
              <motion.ul
                className="space-y-4"
              ...
              >
                {marineLogisticsData.keyFeatures.map((feature, index) => (
          ...
          <div className="mb-20" ref={servicesRef}>
            ...
            >
              Our Marine Logistics Services
            </motion.h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {marineLogisticsData.services.map((service, index) => (
                <motion.div
                  key={index}
          ...
          <div className="mb-20" ref={processRef}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {marineLogisticsData.process.map((step, index) => (
                <motion.div
                  key={index}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center" ref={qualityRef}>
            ...
            >
              ...
              <p className="text-lg text-muted-foreground mb-6">{marineLogisticsData.qualityAssurance.description}</p>
              <motion.ul
                ...
                animate={qualityInView ? "visible" : "hidden"}
              >
                {marineLogisticsData.qualityAssurance.standards.map((standard, index) => (
                  <motion.li key={index} className="flex items-start" variants={itemFadeIn}>
                    <CheckCircle2 className="h-5 w-5 text-primary mt-1 mr-3 flex-shrink-0" />
                    <div>
      ...
      >
        <div className="px-4 text-center">
          <h2 className="text-3xl text-white font-bold mb-6">Emergency Logistics Support</h2>
          <p className="text-lg text-white mb-8 max-w-2xl mx-auto">{marineLogisticsData.emergencyServices}</p>
          <motion.div
          ...
          >
            <motion.div variants={itemFadeIn} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button variant="secondary" size="lg" asChild>
                <Link href="/contact">Request Emergency Support</Link>
              </Button>
            </motion.div>
            <motion.div variants={itemFadeIn} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button variant="outline" className="text-blue border-white hover:bg-white/10" size="lg" asChild>
                <a href={`tel:${marineLogisticsData.emergencyPhone}`}>{marineLogisticsData.emergencyPhone}</a>
              </Button>
            </motion.div>
          </motion.div>
        </div>
