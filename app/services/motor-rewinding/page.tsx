    ...
    <>
      ...
      >
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            ...
            >
              Expert motor repair, rewinding, and maintenance services to extend service life and improve performance
            </motion.p>
          </div>
        </div>
      </motion.section>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20" ref={overviewRef}>
            <motion.div initial="hidden" animate={overviewInView ? "visible" : "hidden"} variants={fadeInUp}>
              <h2 className="text-3xl font-bold mb-6">Comprehensive Motor Services</h2>
              <p className="text-lg text-muted-foreground mb-6">{motorRewindingData.overview}</p>
              <motion.ul
                className="space-y-4"
              ...
              >
                {motorRewindingData.keyFeatures.map((feature, index) => (
          ...
          <div className="mb-20" ref={servicesRef}>
            ...
            >
              Our Motor Rewinding Services
            </motion.h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {motorRewindingData.services.map((service, index) => (
                <motion.div
                  key={index}
                ...
                >
                  <div className="relative h-48">
                    <Image
                      src={service.image || `/images/img/motor-rewinding-${index + 1}.webp`}
                      alt={service.title}
                      fill
          ...
          <div className="mb-20" ref={processRef}>
            ...
            >
              Our Rewinding Process
            </motion.h2>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {motorRewindingData.process.map((step, index) => (
                <motion.div
                  key={index}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center" ref={qualityRef}>
            ...
            >
              ...
              <p className="text-lg text-muted-foreground mb-6">{motorRewindingData.qualityAssurance.description}</p>
              <motion.ul
                ...
                animate={qualityInView ? "visible" : "hidden"}
              >
                {motorRewindingData.qualityAssurance.standards.map((standard, index) => (
                  <motion.li key={index} className="flex items-start" variants={itemFadeIn}>
                    <CheckCircle2 className="h-5 w-5 text-primary mt-1 mr-3 flex-shrink-0" />
                    <div>
              ...
              >
                <Button asChild>
                  <Link href="/contact">Request Motor Service Quote</Link>
                </Button>
              </motion.div>
            </motion.div>
            <motion.div
      ...
      >
        <div className="px-4 text-center">
          <h2 className="text-3xl text-white font-bold mb-6">Emergency Motor Repair</h2>
          <p className="text-lg text-white mb-8 max-w-2xl mx-auto">{motorRewindingData.emergencyServices}</p>
          <motion.div
          ...
          >
            ...
            <motion.div variants={itemFadeIn} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button variant="outline" className="text-blue border-white hover:bg-white/10" size="lg" asChild>
                <a href={`tel:${motorRewindingData.emergencyPhone}`}>{motorRewindingData.emergencyPhone}</a>
              </Button>
            </motion.div>
          </motion.div>
        </div>
